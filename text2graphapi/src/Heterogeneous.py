...
class Heterogeneous(Graph.Graph):
    """This module generate a Heterogeneous graph from raw text 
        ...
        :param int language: language for text preprocessing, default=en (lang: en, es)
        :param bool apply_prep: flag to exec text preprocessing, default=true
    ...
    def _text_normalize_2(self, text_docs: dict) -> list:
        ...
        for i in range(len(text_docs)):
            if self.apply_prep == True:
                prep_text = self.prep.preprocessing_pipeline(text_docs[i]['doc'])
            else:
                prep_text = text_docs[i]['doc']
